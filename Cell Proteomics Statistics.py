...
##Unique Proteins
#The below code performs statistics on the relative abundance data for each unique protein
...
datastats['corrected_p_values'] = multipletests(datastats['T-test_p_value'], method='fdr_bh')[1]
# Write DataFrame to CSV
datastats.to_csv("/Users/chelseahughes/Desktop/PTM Proteomic Analysis/Protein Abundance/ProteinDatastatsNormvsAnox.csv", index=False)
...
datastats= data.iloc[:,0:8]
datastats["Normoxic_Average"]=(data.iloc[:,9:15].sum(axis=1)/data.iloc[:,22:28].sum(axis=1))*100
datastats["Recovery_Average"]=(data.iloc[:,15:21].sum(axis=1)/data.iloc[:,28:34].sum(axis=1))*100
datastats=datastats.replace(np.inf, 0)
datastats['log2FC'] = np.log2(datastats['Recovery_Average'] / (datastats['Normoxic_Average']+.0000001)) #.0000001 added to remove zeroes
datastats=datastats.replace(-np.inf, 0)
# datastats['Fvalue'],datastats['Anova_Pvalue'] = f_oneway(data.iloc[:,8:14],data.iloc[:,14:20],axis=1)
...
datastats['corrected_p_values'] = multipletests(datastats['T-test_p_value'], method='fdr_bh')[1]
# Write DataFrame to CSV
datastats.to_csv("/Users/chelseahughes/Desktop/PTM Proteomic Analysis/Protein Abundance/ProteinDatastatsRecovvsNorm.csv", index=False)
...
datastats['corrected_p_values'] = multipletests(datastats['T-test_p_value'], method='fdr_bh')[1]
# Write DataFrame to CSV
datastats.to_csv("/Users/chelseahughes/Desktop/PTM Proteomic Analysis/Protein Abundance/ProteinDatastatsRecovvsAnox.csv", index=False)
